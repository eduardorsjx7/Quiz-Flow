...
router.post('/:faseId/desbloquear/:usuarioId', authenticate, requireAdmin, (req, res, next) => getFaseController().desbloquearFaseParaUsuario(req, res, next));
router.post('/:faseId/desbloquear-todos', authenticate, requireAdmin, (req, res, next) => getFaseController().desbloquearFaseParaTodos(req, res, next));
router.delete('/:faseId/bloquear/:usuarioId', authenticate, requireAdmin, (req, res, next) => getFaseController().bloquearFaseParaUsuario(req, res, next));
router.get('/:faseId/usuarios-desbloqueados', authenticate, requireAdmin, (req, res, next) => getFaseController().listarUsuariosComDesbloqueio(req, res, next));
